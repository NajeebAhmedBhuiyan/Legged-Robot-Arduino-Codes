/* -----------------------------------------------------------------------------
  - Project: Remote control Crawling robot (handshake-enabled)
  - Original: panerqiang@sunfounder.com
  - Modified: Adds a DONE ack after each motion so RPi can handshake
  ------------------------------------------------------------------------------
  Note: Keep FlexiTimer2.h/.cpp in the same Arduino project.
-------------------------------------------------------------------------------*/
...
void loop()
{
  if (Serial.available() > 0)
  {
    data = Serial.read();
    // Echo for debug:
    Serial.print("Rcv:"); Serial.println(data);

    // Each branch executes motion and then returns "DONE" when finished
    if (data == 'F')
    {
      step_forward();
      Serial.println("DONE");
    ...
    // clear any leftover bytes so we start fresh for the next command
    while (Serial.available()) { Serial.read(); }
  }
}

/* ---------- motion and utility functions (unchanged original code) ---------- */
...
void set_site(int leg, float x, float y, float z)
{
  ...
  if (length == 0) length = 0.0001; // prevent div by zero
